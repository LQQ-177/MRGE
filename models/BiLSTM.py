...
class BiLSTM(nn.Module):
	def __init__(self, config):
		...
		word_vec_size = config.data_word_vec.shape[0]#不同的词数
		self.word_emb = nn.Embedding(word_vec_size, config.data_word_vec.shape[1])#词嵌入
		...
		hidden_size = 128
		input_size = config.data_word_vec.shape[1]#词表示维度
		if self.use_entity_type:
			input_size += config.entity_type_size
			self.ner_emb = nn.Embedding(7, config.entity_type_size, padding_idx=0)#一共6类+什么也不是？
