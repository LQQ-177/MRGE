...
class Config(object):
	def __init__(self, args):
		...
		self.max_length = 512#最大词种类、共指最大数？
		self.pos_num = 2 * self.max_length#位置种类
		self.entity_num = self.max_length#实体种类
		self.relation_num = 97#关系种类

		self.coref_size = 20#共指嵌入维度
		self.entity_type_size = 20#实体类型嵌入维度
		self.max_epoch = 20
		...
		self.train_prefix = args.train_prefix#输出提示词，train.py有设置
		self.test_prefix = args.test_prefix#输出提示词，train.py有设置
	...
	def load_train_data(self):
		...
		self.train_batches = ins_num // self.batch_size#计算一共多好个batch
	...
	def get_train_batch(self):
		random.shuffle(self.train_order)#随机打乱顺序
	...
	def train(self, model_pattern, model_name):
		...
		ori_model.cuda()
		model = nn.DataParallel(ori_model)#用多个GPU来加速训练

		optimizer = optim.Adam(filter(lambda p: p.requires_grad, model.parameters()))#对网络中固定部分参数进行训练
		# nll_average = nn.CrossEntropyLoss(size_average=True, ignore_index=IGNORE_INDEX)
		BCE = nn.BCEWithLogitsLoss(reduction='none')#在BCELoss上加了个sigmoid函数
